...
def main():
    ...
    report_path = REPORT_DIR / f"annotation_sample_selection_{timestamp}.txt"
    manifest_path = OUTPUT_DIR / f"annotation_sample_manifest_{timestamp}.json"
    ...
    lines.append("One debate per legislature using median utterance proximity.")
    lines.append("")

    for item in selected:
        leg = sanitize_legislature(item["legislature"])

        new_name = f"{leg} - {item['file_name']}"
