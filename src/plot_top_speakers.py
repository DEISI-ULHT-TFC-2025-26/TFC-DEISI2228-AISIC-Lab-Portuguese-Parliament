...
OUTPUT_FILE = METRICS_DIR / "top_speakers_bar_chart.png"
...
def save_top_speakers_chart(data):
    top_speakers = data.get("top_speakers", {})

    fig, ax = plt.subplots(figsize=(14, 8))

    if top_speakers:
        plot_horizontal_bar_chart(ax, "TOP SPEAKERS (Top 20)", top_speakers, max_items=20)
    else:
        ax.axis('off')

    plt.tight_layout()
    plt.savefig(OUTPUT_FILE, dpi=300, bbox_inches='tight')
    print(f"Gráfico de Top Speakers guardado em: {OUTPUT_FILE}")
